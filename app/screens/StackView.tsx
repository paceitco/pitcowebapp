...
export const StackView = ({ navigation }: any) => {
    ...
    const renderItem = ({ item }: any) => (
        <SafeAreaView key={ "stack-view-container-" + item?.id } style={styles.gridContainer}>
            <View key={"container-column-a-" + item?.id} className='col-lg-3' style={styles.gridItem}>
                <Image
                    source={{ uri: 'http://localhost:8081/' + item?.imageUrl }}
                    alt={ item?.imageName }
                    // style={[ styles.gridImage, { width: item[0]?.imageWidth, height: item[0]?.imageHeight, marginTop: item[0]?.marginTop || 0 }]}
                    // style={[styles.image, Platform.OS == 'web' ? {} : { width: '40%', height: '40%'}]}
                    style={[styles.image, Platform.OS == 'web' ? { width: item?.imageWidth, height: item?.imageHeight, marginTop: item?.marginTop || 0 } : { width: '40%', height: '40%'}]}
                    resizeMode="cover"
                />
                <HeaderTitle key={"header-column-a-" + item?.id} style={{ textAlign: 'center' }}>{ item?.imageName }</HeaderTitle>
            </View>
        </SafeAreaView>
    );
    ...
    return (
        ...
        <View>
            <HeaderTitle style={{ textAlign: 'center', paddingBottom: 30, fontWeight: 'bold', color: '#fff' }}>Stack History</HeaderTitle>
